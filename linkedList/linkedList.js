...
//add a value (in a newNode) to a middle of LinkedList
LinkedList.prototype.addToMiddle = function (value, position){
    //catch position tail or head:
    if (position==0) 
        return this.addToHead(value);
    else{//traversing to position -1 
        let k = 1;
        let tempNodeP = new Node (null,null,null);
        let tempNodeQ = new Node (null,null,null);
        tempNodeP = this.head;
        tempNodeQ = this.head;
        while ( (tempNodeP!= null) && ( k < position-1)){
            k += 1;
            tempNodeQ = tempNodeP;
            tempNodeP = tempNodeP.next; 
        }
        if(tempNodeP==null){//insert at the end
            return this.addToTail(value); 
        }
        else{//insert at the middle
            const newMiddleNode  = new Node (value,null , tempNodeP);
            tempNodeQ.next = newMiddleNode;
        }
    }
}
...
console.log("Before add the middle " );
list.print();
list.addToMiddle(3,1);
console.log("After add the middle " );
list.print();
